...
)
{
    FRESULT res;
      ...
      for (;;) {
          res = f_readdir(&dir1, &fno1);   // Read a directory item

         if (res || !fno1.fname[0]) break;  // Error or end of dir

         if (fno1.fattrib & AM_DIR)
         {
           printf("   <dir0> ----  %s \r\n", fno1.fname);
            res = f_opendir(&dir2, fno1.fname);
           for (;;) {
          res = f_readdir(&dir2, &fno);   // Read a directory item
