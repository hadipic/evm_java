...
/* evm native */
static void jsR_callcnative(js_State *J, int n, int min, js_CNative F)
{
        int i;
        js_Value v;
        js_Value pthis = *js_tovalue(J, -n - 1);
        js_Value *args = js_tovalue(J, -n);
	...
	if (obj->type == JS_CFUNCTION) {
		...
		jsR_pushtrace(J, obj->u.c.name, "native", 0);
                ...
                if( obj->u.c.function )
                    ...
                    jsR_callcnative(J, n, obj->u.c.length, obj->u.c.native);
                }
		--J->tracetop;
	}
