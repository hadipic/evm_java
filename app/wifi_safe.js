...
console.log("📡 Initializing WiFi in SAFE mode...");
try {
...
function updateNetworkList() {
    ...
    if (networks.length === 0) {
        lvgl.label_set_text(networkLabels[0], "No networks found");
    ...
    switch(currentMode) {
        ...
        case MODES.INFO:
            try {
                var status = WiFi.status();
                var apIP = WiFi.apIP();
                var staIP = WiFi.staIP();
                infoText = "WiFi: " + (status ? "Ready" : "Error");
        ...
        case MODES.ABOUT:
            infoText = "WiFi Scanner v2.0 | Safe Mode";
            break;
    }
...
function performScan() {
    ...
    try {
        ...
        if (scanResult && scanResult.length > 0) {
            ...
            for (var i = 0; i < Math.min(networks.length, 3); i++) {
                var net = networks[i];
...
function autoScanIfNeeded() {
    ...
    if (currentMode === MODES.SCAN && currentTime - lastAutoScanTime > 30000) {
...
function initializeApp() {
    ...
    console.log("📡 Mode: Safe WiFi Scanner");
...
function mainLoop() {
    ...
    while (appRunning) {
        ...
        // شبیه‌سازی دکمه برای تست - هر 80 حلقه یک دکمه تصادفی
        var upPressed = false;
